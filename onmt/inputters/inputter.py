...
def get_fields(data_type, n_src_features, n_tgt_features, tree=False):
    """
    Args:
...
def make_features(batch, side, data_type='text'):
    ...
    if data_type == 'text':
        ...
        if side + '_parse' in batch.__dict__:
            data = (data, batch.__dict__[side + '_parse'])
        return data
    else:
        return levels[0]
...
def build_dataset(fields, data_type, src_data_iter=None, src_path=None,
                  ...
                  src_parse=None, src_parse_iter=None, src_seq_length=0,
                  tgt_seq_length=0, src_seq_length_trunc=0,
                  tgt_seq_length_trunc=0, dynamic_dict=True, sample_rate=0,
    ...
    def _make_examples_nfeats_tpl(data_type, src_data_iter, src_path, src_dir,
                                  src_parse, src_parse_iter,
                                  src_seq_length_trunc, sample_rate,
                                  window_size, window_stride, window,
                                  normalize_audio, image_channel_size=3):
        """
        Process the corpus into (example_dict iterator, num_feats) tuple
        on source side for different 'data_type'.
        """
        src_parse_ex_iter = None
        if data_type == 'text':
            src_examples_iter, num_src_feats = \
                TextDataset.make_text_examples_nfeats_tpl(
                    src_data_iter, src_path, src_seq_length_trunc, "src")
            if src_parse_iter is not None or src_parse is not None:
                src_parse_ex_iter = TextDataset.make_text_examples_parse_tpl(
                    src_parse_iter, src_parse, "src")
        ...
        return src_examples_iter, num_src_feats, src_parse_ex_iter

    src_examples_iter, num_src_feats, src_parse_ex_iter = \
    ...
    if data_type == 'text':
        dataset = TextDataset(fields, src_examples_iter, tgt_examples_iter,
                              ...
                              src_parse_iter=src_parse_ex_iter,
