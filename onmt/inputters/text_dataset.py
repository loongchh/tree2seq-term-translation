...
from onmt.inputters.dataset_base import (DatasetBase, UNK_WORD,
                                         PAD_WORD, BOS_WORD, EOS_WORD, PAD)
...
class TextDataset(DatasetBase):
    ...
    def __init__(self, fields, src_examples_iter, tgt_examples_iter,
                 ...
                 src_seq_length=0, tgt_seq_length=0, src_parse_iter=None,
        ...
        if src_parse_iter is not None:
            src_examples_iter = (self._join_dicts(src, src_t) for src, src_t in
                                 zip(src_examples_iter, src_parse_iter))
    ...
    @staticmethod
    def make_text_examples_parse_tpl(parse_iter, parse_path, side):
        """
        Args:
            parse_iter(iterator): an iterator (or None) that we can loop over
                to read examples.
                It may be an openned file, a string list etc...
            parse_path(str): path to file or None
            path (str): location of a src or tgt file.
            side (str): "src" or "tgt".
        ...
        if parse_iter is None:
            if parse_path is not None:
                parse_iter = TextDataset.make_text_iterator_from_file(
                    parse_path)
            else:
                return (None, 0)
        ...
        # All examples have same number of features, so we peek first one
        # to get the num_feats.
        examples_parse_iter = TextDataset.make_parse_examples(parse_iter, side)

        return examples_parse_iter
    ...
    @staticmethod
    def make_parse_examples(parse_iter, side):
        """
        Args:
            parse_iter (iterator): iterator of parse sequences
            side (str): "src" or "tgt".

        Yields:
            (word, features, nfeat) triples for each line.
        """
        for i, line in enumerate(parse_iter):
            tokens = [int(w) for w in line.strip().split()]
            example_dict = {side + '_parse': tokens, "indices": i}
            yield example_dict
    ...
    def get_fields(n_src_features, n_tgt_features):
        ...
        fields["src_parse"] = torchtext.data.Field(
            pad_token=PAD, use_vocab=False, include_lengths=False)
