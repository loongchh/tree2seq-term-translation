...
from onmt.decoders.tree_decoder import Tree2SeqDecoder
...
def build_encoder(opt, embeddings):
    ...
    elif opt.encoder_type == "tree":
        return TreeEncoder(opt.rnn_type, opt.brnn, opt.enc_layers,
...
def build_decoder(opt, embeddings):
    ...
    """
    if opt.encoder_type == "tree":
        return Tree2SeqDecoder(opt.rnn_type, opt.brnn, opt.dec_layers,
                               opt.rnn_size, opt.global_attention,
                               opt.global_attention_function,
                               opt.coverage_attn, opt.context_gate,
                               opt.copy_attn, opt.dropout, embeddings,
                               opt.reuse_copy_attn, opt.tree_combine)
    ...
    if checkpoint is not None:
        ...
        if hasattr(model.encoder, 'embeddings'):
            model.encoder.embeddings.load_pretrained_vectors(
                src_dict.vectors, model_opt.fix_word_vecs_src)
        if hasattr(model.decoder, 'embeddings'):
            model.decoder.embeddings.load_pretrained_vectors(
                tgt_dict.vectors, model_opt.fix_word_vecs_tgt)
