...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    kwargs = {k: getattr(opt, k)
              for k in ["beam_size", "n_best", "max_length", "min_length",
                        ...
                        "report_f1", "data_type", "replace_unk", "gpu",
                        "verbose", "json", "fast", "sample_rate",
...
class Translator(object):
    ...
    def translate(self,
        ...
        for batch in data_iter:
            ...
            for trans in translations:
                ...
                if self.verbose or self.json:
                    sent_number = next(counter)
                    output = trans.log(sent_number, json_output=self.json)
                    if self.logger:
        ...
        if self.report_score:
            ...
            if tgt_path is not None:
                ...
                if self.report_rouge:
                    msg = self._report_script(tgt_path, script='test_rouge.py')
                    if self.logger:
                        self.logger.info(msg)
                    else:
                        print(msg)
                if self.report_f1:
                    msg = self._report_script(tgt_path, script='test_f1.py')
                    if self.logger:
                        self.logger.info(msg)
                    else:
                        print(msg)
    ...
    def _report_script(self, tgt_path, script='test_rouge.py'):
        import subprocess
        path = os.path.split(os.path.realpath(__file__))[0]
        script_path = os.path.join(path, 'tools', script)
        res = subprocess.check_output(
            "python {:s} -r {:s} -c STDIN".format(
            script_path, tgt_path),
            shell=True,
