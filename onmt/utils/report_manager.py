...
def build_report_manager(opt):
    if opt.tensorboard_dir:
        ...
        writer = SummaryWriter(os.path.join(opt.tensorboard_dir,
    ...
    report_mgr = ReportMgr(opt.report_every, start_time=-1,
                           tensorboard_writer=writer, json_output=opt.json)
    return report_mgr
...
class ReportMgr(ReportMgrBase):
    ...
    def _report_training(self, step, num_steps, learning_rate,
        ...
        """
        if self.json:
            report_stats.output_json(step, num_steps,
    ...
    def _report_step(self, lr, step, train_stats=None, valid_stats=None):
        ...
        if train_stats is not None:
            if self.json:
                print('{{"metric": "Train perplexity", "value": {}, "step": {}}}'.format(train_stats.ppl(), step))
                print('{{"metric": "Train accuracy", "value": {}, "step": {}}}'.format(train_stats.accuracy(), step))
            else:
        ...
        if valid_stats is not None:
            if self.json:
                print('{{"metric": "Valid perplexity", "value": {}, "step": {}}}'.format(valid_stats.ppl(), step))
                print('{{"metric": "Valid accuracy", "value": {}, "step": {}}}'.format(valid_stats.accuracy(), step))
            else:
