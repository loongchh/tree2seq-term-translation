...
import torch
import torchtext
from onmt.utils.logging import init_logger, logger, try_debugger
...
def load_pretrained_vectors(vocab, embed_path, random_init=True):
    """Load pretrained vectors from embed file to vocab. """
    if embed_path is not None:
        unk_init = torch.rand_like if random_init else None
        cache_path = os.path.dirname(embed_path)
        embed_vec = torchtext.vocab.Vectors(
            embed_path, unk_init=unk_init, cache=cache_path)
        vocab.load_vectors(embed_vec)
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    if corpus_type == 'train':
        ...
        src_parse = opt.train_src_parse
    elif corpus_type == 'test':
        src_corpus = opt.test_src
        tgt_corpus = opt.test_tgt
        src_parse = opt.test_src_parse
    else:
        src_corpus = opt.valid_src
        tgt_corpus = opt.valid_tgt
        src_parse = opt.valid_src_parse
...
def build_save_vocab(train_dataset, fields, opt):
    ...
    load_pretrained_vectors(fields['src'].vocab, opt.pre_word_vecs_src)
    load_pretrained_vectors(fields['tgt'].vocab, opt.pre_word_vecs_tgt)
...
def main(opt):
    ...
    logger.info("Building & saving testing data...")
    test_dataset_files = build_save_dataset('test', fields, opt)

    logger.info("Building & saving vocabulary...")
    dataset_files = train_dataset_files + valid_dataset_files + test_dataset_files
    build_save_vocab(dataset_files, fields, opt)
